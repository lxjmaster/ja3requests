...
class TLSRecordLayer:
    ...
    def set_keys(self, client_write_key, server_write_key, 
                 client_write_mac_key, server_write_mac_key,
                 client_write_iv, server_write_iv, cipher_suite):
    ...
    def encrypt_application_data(self, data: bytes, content_type: int = 23) -> bytes:
        ...
        try:
            ...
            if self.cipher_suite in [0x002F, 0x0035]:  # SHA-1 based
                mac = hmac.new(self.client_write_mac_key, mac_data, hashlib.sha1).digest()
            else:  # SHA-256 based
                mac = hmac.new(self.client_write_mac_key, mac_data, hashlib.sha256).digest()
            ...
            record_header = struct.pack('!BBBH', content_type, 3, 3, len(encrypted_data))
    ...
    def decrypt_application_data(self, record_data: bytes) -> Tuple[bytes, int]:
        ...
        content_type, major_version, minor_version, length = struct.unpack('!BBBH', record_data[:5])
        ...
        try:
            ...
            # Decrypt
            plaintext_with_mac = AESCipher.decrypt_cbc(ciphertext, self.server_write_key, iv)
            ...
            if self.cipher_suite in [0x002F, 0x0035]:  # SHA-1 based
                expected_mac = hmac.new(self.server_write_mac_key, mac_data, hashlib.sha1).digest()
            else:  # SHA-256 based
                expected_mac = hmac.new(self.server_write_mac_key, mac_data, hashlib.sha256).digest()
        ...
        while True:
            ...
            if len(self.receive_buffer) >= 5:
                ...
                if len(self.receive_buffer) >= total_record_length:
                    ...
                    try:
                        decrypted_data, content_type = self.record_layer.decrypt_application_data(record)
