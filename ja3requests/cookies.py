...
class MockRequest:
    """Wraps a `Ja3Requests.Request` to mimic a `urllib2.Request`.

    The code in `CookieJar` expects this interface in order to correctly
    ...
    def add_header(self, key, val):
        """cookie has no legitimate use for this method; add it back if you find one."""
...
class MockResponse:
    """
    Basically, expose the parsed HTTP headers from the server response
    the way `cookie` expects to see them.
    """

    def __init__(self, response):
        """Make a MockResponse for `cookie` to read.
...
def extract_cookies_to_jar(jar, request, response):
    ...
    :param jar: CookieJar (not necessarily a RequestsCookieJar)
    :param request: our own Ja3Requests.Request object
    :param response: our own Ja3Requests.Response object
    """
...
class Ja3RequestsCookieJar(CookieJar, MutableMapping):
    """Compatibility class; is a CookieJar, but exposes a dict
    interface.
    ...
    def iterkeys(self):
        ...
        .. see also:: itervalues() and iteritems().
        """
    ...
    def itervalues(self):
        ...
        .. see also:: iterkeys() and iteritems().
        """
        for cookie in iter(self):
            yield cookie.value
    ...
    def iteritems(self):
        ...
        .. see also:: iterkeys() and itervalues().
        """
        for cookie in iter(self):
    ...
    def __getstate__(self):
        ...
        # remove the unpickable RLock object
...
def create_cookie(name, value, **kwargs):
    ...
    if bad_args:
        raise TypeError(
            f"create_cookie() got unexpected keyword arguments: {list(bad_args)}"
        )
...
def cookiejar_from_dict(cookie_dict, cookiejar=None, overwrite=True):
    ...
    if cookiejar is None:
        cookiejar = Ja3RequestsCookieJar()
...
def merge_cookies(cookiejar, _cookies):
    ...
    elif isinstance(_cookies, CookieJar):
        try:
            cookiejar.update(_cookies)
        except AttributeError:
            for cookie_in_jar in _cookies:
